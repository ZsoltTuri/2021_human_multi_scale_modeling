function counter = couple_script(var)
arguments
    var.param_file
    var.quasipotentials_file
end
%% Import parameters
fid = fopen(var.param_file);
tline = fgetl(fid);
while ischar(tline)
    eval(tline);
    tline = fgetl(fid);
...
% for all of them
counter = 0; 
limit = 1000; % in certain locations, the while loop will stuck, hence, apply limit
while any(isnan(psi))
    counter = counter + 1;
...
save([respath filesep var.quasipotentials_file], 'psi','-ascii');
